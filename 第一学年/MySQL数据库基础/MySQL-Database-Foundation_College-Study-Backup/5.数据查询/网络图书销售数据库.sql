...
SELECT SUN(sell.订购册数) AS '总销售量'
    FROM book,sell
    WHERE book.图书编号=sell.图书编号
        AND book.图书名称='网页程序设计';
SELECT book.书名,members.姓名,sell.订购册数
    FROM book,members,sell
    WHERE sell.订购册数>5
        AND book.图书编号=sell.图书编号
        AND sell.用户号=members.用户号;
SELECT members.姓名,sell.订购册数
    FROM book,members,sell
    WHERE sell.订购册数>5
        AND book.图书名称='网页程序设计'
        AND book.图书编号=sell.图书编号
