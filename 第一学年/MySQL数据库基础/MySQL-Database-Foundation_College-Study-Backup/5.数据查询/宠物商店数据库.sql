...
SELECT DISTINCT itemid,unitprice
	FROM lineitem;
SELECT *,quantity*unitprice AS 商品金额
	FROM lineitem;
SELECT fullname,
	CASE
		WHEN sex='男' THEN 1
...
SELECT name,
	CASE
		...
		WHEN unitcost BETWEEN 1000 AND 2000 THEN '低价商品'
		WHEN unitcost>2000 THEN '高档商品'
		END AS unitcost
	FROM product;
...
SELECT *
	FROM account
	WHERE fullname LIKE '吴%' OR fullname REGEXP '吴.*';
SELECT *
	FROM orders
	WHERE totalprice BETWEEN 200 AND 500;
SELECT *
	FROM product
	WHERE productid LIKE '%W___' OR productid REGEXP '.*W...';